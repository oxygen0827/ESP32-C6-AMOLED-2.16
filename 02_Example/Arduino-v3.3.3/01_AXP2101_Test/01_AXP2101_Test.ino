...
void Axp2101_isChargingTask(void *arg) {
  for (;;) {
    ...
    Serial.printf("isCharging: %s\n", axp2101.isCharging() ? "YES" : "NO");
    uint8_t charge_status = axp2101.getChargerStatus();
    if (charge_status == XPOWERS_AXP2101_CHG_TRI_STATE) {
      Serial.printf("Charger Status: tri_charge\n");
    } else if (charge_status == XPOWERS_AXP2101_CHG_PRE_STATE) {
      Serial.printf("Charger Status: pre_charge\n");
    } else if (charge_status == XPOWERS_AXP2101_CHG_CC_STATE) {
      Serial.printf("Charger Status: constant charge\n");
    } else if (charge_status == XPOWERS_AXP2101_CHG_CV_STATE) {
      Serial.printf("Charger Status: constant voltage\n");
    } else if (charge_status == XPOWERS_AXP2101_CHG_DONE_STATE) {
      Serial.printf("Charger Status: charge done\n");
    } else if (charge_status == XPOWERS_AXP2101_CHG_STOP_STATE) {
      Serial.printf("Charger Status: not charge\n");
    }
    Serial.printf("getBattVoltage: %d mV\n", axp2101.getBattVoltage());
  }
}
