#!/usr/bin/env python3
"""Clara board-behaviour simulator: replay the ESP32-C6 firmware's exact
network sequence on this machine to isolate board vs backend vs network-path
problems.

Sequence per boot:
  1. Extract the pinned CA chain compiled into clara_ca_chain.h.
...
Each WebSocket is tried under three trust modes so failures localise fast:
  pinned : verify against kClaraCaChainPem exactly like mbedtls on the board
...
HEADER = os.path.join(PROJECT_ROOT, "03_Clara_C6", "main", "clara_ca_chain.h")
LOCAL = os.path.join(PROJECT_ROOT, "03_Clara_C6", "sdkconfig.local")
PEM_OUT = os.path.join(SCRIPT_DIR, "clara_ca_chain.pem")
...
def extract_pem():
    ...
    if not m:
        sys.exit("cannot parse clara_ca_chain.h")
    pem = m.group(1).strip() + "\n"
...
def http_base():
    base = read_local("CONFIG_CLARA_API_BASE_URL") or ""
    if not base:
        sys.exit("sdkconfig.local missing CONFIG_CLARA_API_BASE_URL")
    return base.rstrip("/")
...
def create_session(base):
    ...
    try:
        r = requests.post(url, json={"topic": read_local("CONFIG_CLARA_TOPIC") or "meeting"},
                          timeout=TIMEOUT)
...
def main():
    ...
    print(f"=== Clara WSS simulator  base={base} ===")
    extract_pem()
    show_leaf_cert(host)
