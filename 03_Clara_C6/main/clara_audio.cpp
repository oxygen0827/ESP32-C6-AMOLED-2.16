...
#endif
#endif
#ifndef CLARA_AUDIO_HAS_MP3
#define CLARA_AUDIO_HAS_MP3 0
#endif

namespace {

constexpr const char *TAG = "clara_audio";
...
struct AudioState {
    ...
    uint32_t sample_rate = CLARA_AUDIO_SAMPLE_RATE;
    uint8_t channels = CLARA_AUDIO_CODEC_CHANNELS;
    uint8_t bits = CLARA_AUDIO_BITS_PER_SAMPLE;
                                          ...
                                          const esp_audio_simple_dec_info_t &info)
{
    ...
    if (info.sample_rate == CLARA_AUDIO_SAMPLE_RATE) {
        ...
        const size_t output_capacity = (input_frames * CLARA_AUDIO_SAMPLE_RATE + info.sample_rate - 1U) /
                                       info.sample_rate + 2U;
...
extern "C" esp_err_t clara_audio_init(const clara_audio_config_t *config)
{
    ...
    s_audio.sample_rate = config->sample_rate ? config->sample_rate : CLARA_AUDIO_SAMPLE_RATE;
    s_audio.channels = config->codec_channels ? config->codec_channels : CLARA_AUDIO_CODEC_CHANNELS;
    s_audio.bits = config->bits_per_sample ? config->bits_per_sample : CLARA_AUDIO_BITS_PER_SAMPLE;
...
extern "C" esp_err_t clara_audio_init_from_codec(void *codec_port)
{
    if (!codec_port) return ESP_ERR_INVALID_ARG;
    auto *codec = static_cast<CodecPort *>(codec_port);
    clara_audio_config_t config = {};
    config.playback = codec->Get_audio_codec_speaker();
    config.capture = codec->Get_audio_codec_microphone();
    config.sample_rate = CLARA_AUDIO_SAMPLE_RATE;
    config.codec_channels = CLARA_AUDIO_CODEC_CHANNELS;
    config.bits_per_sample = CLARA_AUDIO_BITS_PER_SAMPLE;
    config.capture_channel = kMixAllChannels;
    config.output_volume = -1;
    config.input_gain_db = -1.0f;
    return clara_audio_init(&config);
}

extern "C" esp_err_t clara_audio_deinit(void)
{
#if CLARA_AUDIO_HAS_MP3
    clara_audio_mp3_end();
#endif
...
extern "C" bool clara_audio_is_ready(void)
{
    return s_audio.ready && s_audio.capture && s_audio.playback;
}

extern "C" esp_err_t clara_audio_capture_start(void)
{
    if (!clara_audio_is_ready() || !lock(s_audio.capture_lock)) return ESP_ERR_INVALID_STATE;
    esp_err_t err = ensure_capture_open_locked();
    unlock(s_audio.capture_lock);
    return err;
}

extern "C" esp_err_t clara_audio_capture_stop(void)
{
    if (!clara_audio_is_ready() || !lock(s_audio.capture_lock)) return ESP_ERR_INVALID_STATE;
...
extern "C" esp_err_t clara_audio_playback_start(void)
{
    if (!clara_audio_is_ready() || !lock(s_audio.playback_lock)) return ESP_ERR_INVALID_STATE;
    esp_err_t err = ensure_playback_open_locked();
    unlock(s_audio.playback_lock);
    return err;
}

extern "C" esp_err_t clara_audio_playback_stop(void)
{
    if (!clara_audio_is_ready() || !lock(s_audio.playback_lock)) return ESP_ERR_INVALID_STATE;
...
extern "C" int clara_audio_read_mono_pcm16(int16_t *dst, size_t frames)
{
    if (!dst || frames == 0 || !clara_audio_is_ready()) return -1;
...
extern "C" int clara_audio_read_pcm16(void *dst, size_t bytes)
{
    if (!dst || bytes == 0 || (bytes % sizeof(int16_t)) != 0) return -1;
    const int frames = clara_audio_read_mono_pcm16(static_cast<int16_t *>(dst), bytes / sizeof(int16_t));
    return frames < 0 ? -1 : frames * static_cast<int>(sizeof(int16_t));
}

extern "C" esp_err_t clara_audio_write_pcm16(const int16_t *pcm, size_t frames)
{
    if (!pcm || frames == 0 || !clara_audio_is_ready()) return ESP_ERR_INVALID_ARG;
...
extern "C" esp_err_t clara_audio_play_pcm16(const int16_t *pcm, size_t frames)
{
    return clara_audio_write_pcm16(pcm, frames);
}

extern "C" esp_err_t clara_audio_set_volume(int volume)
{
    if (!clara_audio_is_ready() || !lock(s_audio.playback_lock)) return ESP_ERR_INVALID_STATE;
...
extern "C" int clara_audio_get_volume(void)
{
    return s_audio.volume;
}

extern "C" esp_err_t clara_audio_set_input_gain(float gain_db)
{
    if (!clara_audio_is_ready() || !std::isfinite(gain_db)) return ESP_ERR_INVALID_STATE;
...
{
#if CLARA_AUDIO_HAS_MP3
    if (!clara_audio_is_ready()) return ESP_ERR_INVALID_STATE;
...
extern "C" esp_err_t clara_audio_mp3_write(const uint8_t *data, size_t len, bool end_of_stream)
{
#if CLARA_AUDIO_HAS_MP3
    if ((!data && len != 0) || !clara_audio_is_ready()) return ESP_ERR_INVALID_ARG;
    if (!s_audio.mp3_active) {
        esp_err_t start_err = clara_audio_mp3_start();
    ...
    if (end_of_stream) return clara_audio_mp3_end();
    return ESP_OK;
#else
    (void)data;
    (void)len;
...
extern "C" esp_err_t clara_audio_mp3_end(void)
{
#if CLARA_AUDIO_HAS_MP3
...
extern "C" esp_err_t clara_audio_play_mp3(const uint8_t *data, size_t len)
{
    esp_err_t err = clara_audio_mp3_start();
    if (err != ESP_OK) return err;
    return clara_audio_mp3_write(data, len, true);
}
