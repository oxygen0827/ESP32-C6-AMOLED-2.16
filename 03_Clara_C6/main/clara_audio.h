...
/*
 * Clara audio adapter for the ESP32-C6 AMOLED board.
 *
...
#define CLARA_AUDIO_SAMPLE_RATE       (16000U)
#define CLARA_AUDIO_PCM_CHANNELS      (1U)
#define CLARA_AUDIO_CODEC_CHANNELS   (2U)
#define CLARA_AUDIO_BITS_PER_SAMPLE  (16U)
#define CLARA_AUDIO_FRAME_SAMPLES    (320U)  /* 20 ms at 16 kHz */
#define CLARA_AUDIO_FRAME_BYTES      (CLARA_AUDIO_FRAME_SAMPLES * sizeof(int16_t))

/** Format and device handles supplied by the board layer. */
typedef struct {
    esp_codec_dev_handle_t playback;
    esp_codec_dev_handle_t capture;
    uint32_t sample_rate;       /* Defaults to CLARA_AUDIO_SAMPLE_RATE. */
    uint8_t codec_channels;     /* Defaults to CLARA_AUDIO_CODEC_CHANNELS. */
    uint8_t bits_per_sample;    /* Defaults to CLARA_AUDIO_BITS_PER_SAMPLE. */
...
} clara_audio_config_t;

/** Initialise from already-created esp_codec_dev handles. */
esp_err_t clara_audio_init(const clara_audio_config_t *config);

/** Convenience C ABI entry point for a C++ CodecPort object (opaque pointer). */
esp_err_t clara_audio_init_from_codec(void *codec_port);

/** Close the devices and release adapter-owned buffers. Handles remain owned by CodecPort. */
esp_err_t clara_audio_deinit(void);
bool clara_audio_is_ready(void);

/** Start/stop logical capture and playback streams. Init starts both streams. */
esp_err_t clara_audio_capture_start(void);
esp_err_t clara_audio_capture_stop(void);
esp_err_t clara_audio_playback_start(void);
esp_err_t clara_audio_playback_stop(void);
 ...
 */
int clara_audio_read_mono_pcm16(int16_t *dst, size_t frames);

/** Byte-oriented alias for WebSocket feed code. Returns bytes read or -1. */
int clara_audio_read_pcm16(void *dst, size_t bytes);

/** Write 16 kHz mono PCM16. The call blocks until the codec accepts the data. */
esp_err_t clara_audio_write_pcm16(const int16_t *pcm, size_t frames);
esp_err_t clara_audio_play_pcm16(const int16_t *pcm, size_t frames);

esp_err_t clara_audio_set_volume(int volume);
int clara_audio_get_volume(void);
 ...
 */
esp_err_t clara_audio_mp3_start(void);
esp_err_t clara_audio_mp3_write(const uint8_t *data, size_t len, bool end_of_stream);
esp_err_t clara_audio_mp3_end(void);
esp_err_t clara_audio_play_mp3(const uint8_t *data, size_t len);

/* Compatibility names used by the original ws_meeting_demo pipeline. */
static inline int clara_audio_recorder_read(void *dst, size_t bytes)
{
    return clara_audio_read_pcm16(dst, bytes);
}
static inline esp_err_t clara_audio_player_write_pcm(const int16_t *pcm, int frames)
{
    return (frames < 0) ? ESP_ERR_INVALID_ARG : clara_audio_write_pcm16(pcm, (size_t)frames);
}
