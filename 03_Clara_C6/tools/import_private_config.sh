...
umask 077
{
    printf 'CONFIG_CLARA_WIFI_SSID=%s\n' "$wifi_ssid"
    printf 'CONFIG_CLARA_WIFI_PASSWORD=%s\n' "$wifi_password"
    printf 'CONFIG_CLARA_API_BASE_URL=%s\n' "$api_base"
    printf 'CONFIG_CLARA_TOPIC=%s\n' "$topic"
} > "$target_file"
chmod 600 "$target_file"
printf 'imported private Clara build configuration (values were not printed)\n'
