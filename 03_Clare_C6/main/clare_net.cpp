...
static void dispatch_ws_json(WsContext *ctx, const char *payload, size_t payload_len)
{
    ...
    if (strcmp(type, "error") == 0) {
        ...
        emit_event(CLARE_NET_EVENT_TRANSCRIPT, text, nullptr, 0,
...
static esp_err_t ws_connect(WsContext *ctx, const char *session_id)
{
    ...
    // Wait must exceed config.network_timeout_ms (60s): destroying the client
    // mid-handshake leaks its half-built TLS context and the retry then fails
    // with MBEDTLS_ERR_PK_ALLOC_FAILED (logs/clare_sw_aes_20260828.log).
